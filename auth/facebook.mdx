...
<Warning>
  **Facebook Login is not yet available for mobile apps.** We are working on this feature. Currently, Facebook Login only works for web apps published to custom domains. Additionally, per Apple's App Store guidelines, Facebook SSO cannot be present in iOS mobile builds until the "Sign in with Apple" feature is released.
</Warning>
